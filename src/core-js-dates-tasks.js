...
function getNextFriday(date) {
  ...
  const dayWeek = currDate.getDay();
  const dayNumber = currDate.getDate();
  const addDays = (12 - dayWeek) % 7 || 7;
  const nextFriday = new Date(currDate.setDate(dayNumber + addDays));
  return nextFriday;
}
...
function isDateInPeriod(date, period) {
  ...
  return (
    currDate >= Date.parse(period.start) && currDate <= Date.parse(period.end)
  );
}
...
function formatDate(date) {
  ...
  const month = currData.getUTCMonth() + 1;
  const day = currData.getUTCDate();
  const year = currData.getUTCFullYear();
  const time = currData.toLocaleTimeString('en-US', { timeZone: 'UTC' });
...
function getWeekNumberByDate(date) {
  ...
  const weekNumber = Math.ceil(
    ((currDate - startDate) / milliseconds + startDate.getDay() + 1) / 7
  );
  return weekNumber;
}
...
function isLeapYear(date) {
  ...
  return currYear % 4 === 0 || currYear % 400 === 0;
}
